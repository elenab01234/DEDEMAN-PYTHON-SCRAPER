...
```bash
python3 -c "from scraper.anaf import get_company_from_anaf; print(get_company_from_anaf('2816464'))"
```

Expect: `denumire == "DEDEMAN SRL"`, `cif == "2816464"`.
...
Expect: `>= 1` jobs scraped, unique URLs, titles present, Dedeman careers board.

## Live — peviitor SOLR

```bash
curl "https://api.peviitor.ro/v1/scraper/jobs/?cif=2816464&rows=500"
```

Expect: `success: true`. Note CIF `2816464` is shared with other peviitor
scrapers (jobviewtrack, ejobs, olx, multijobs, targuldecariere), so the
total count includes their jobs; confirm the scraped Dedeman board URLs
are present.
...
```bash
gh api repos/elenab01234/DEDEMAN-PYTHON-SCRAPER/pages --jq .html_url
curl -s -o /dev/null -w "%{http_code}\n" https://elenab01234.github.io/DEDEMAN-PYTHON-SCRAPER/
```

Expect: `https://elenab01234.github.io/DEDEMAN-PYTHON-SCRAPER/` and HTTP `200`.
...
```bash
curl -s "https://api.peviitor.ro/v1/scraper/jobs/?cif=2816464&rows=500"
```

Check `docs/jobs.md` was regenerated and jobs are visible on https://peviitor.ro (CIF `2816464`).
