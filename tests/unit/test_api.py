...
def test_upsert_jobs_keeps_cif(mock_post):
    ...
    api.upsert_jobs([{"url": "u", "cif": "2816464"}])
    sent = mock_post.call_args.kwargs["json"]
    assert sent[0]["cif"] == "2816464"
...
def test_upsert_company_keeps_id(mock_put):
    ...
    api.upsert_company({"id": "2816464", "company": "DEDEMAN S.R.L."})
    sent = mock_put.call_args.kwargs["json"]
    assert sent["id"] == "2816464"
...
BOARD = "https://recrutare.dedeman.ro/detalii-post?"
...
def test_run_verification_delete_scoped_to_prefix(monkeypatch):
    ...
    api.run_verification("2816464", delete=True, prefix=BOARD)
...
def test_run_verification_delete_requires_prefix(monkeypatch):
    ...
    api.run_verification("2816464", delete=True, prefix=None)
